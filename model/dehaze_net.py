...
######################################################################################################################################
class PFB2(nn.Module):  # physical block
    def __init__(self, channel):
        super(PFB2, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        ...
        self.IFB = Prediction(num_channels=channel)
        self.DFB = Prediction(num_channels=channel)
        self.con = nn.Conv2d(48, 96, 1, padding=0, bias=True)

        self.Con_R = nn.Conv2d(channel // 4, 3, 1, padding=0, bias=True)
        self.Con_L = nn.Conv2d(channel // 4, 1, 1, padding=0, bias=True)
    ...
    def forward(self, x):
        ...
        I_L = self.Con_L(t1)
        I_R = self.Con_R(t2)
        I_L1 = torch.cat((I_L, I_L, I_L), dim=1)
        I_Rtx = I_L1 * I_R
        ...
        return j, I_Rtx, I_L1, I_R


class PFB(nn.Module):  # physical block
    def __init__(self, channel):
        ...
        self.IFB = Prediction(num_channels=channel)
        self.DFB = Prediction(num_channels=channel)
        self.con = nn.Conv2d(channel, int(channel * 2 ** 1), 1, padding=0, bias=True)

        self.Con_R = nn.Conv2d(channel // 2, 3, 1, padding=0, bias=True)
        self.Con_L = nn.Conv2d(channel // 2, 1, 1, padding=0, bias=True)
...
class DRSformer(nn.Module):
    def __init__(self,
        ...
        self.refine_ffp = PFB2(int(dim * 2 ** 1))
    ...
    def forward(self, inp_img):
        ...
        inp_enc_level0, I_Rtx = self.ffp(inp_enc_level1)

        out_enc_level1 = self.encoder_level1(inp_enc_level0)
        features.append(out_enc_level1)

        inp_enc_level2 = self.down1_2(out_enc_level1)
        out_enc_level2 = self.encoder_level2(inp_enc_level2)
        features.append(out_enc_level2)
        ...
        out_dec_level1, I_Rtx2, I_L, I_R = self.refine_ffp(out_dec_level1)

        out_dec_level1 = self.output(out_dec_level1) + inp_img

        return out_dec_level1, I_Rtx, I_Rtx2, features
